...
YOURPLACE_IPFS_GATEWAY="${YOURPLACE_IPFS_GATEWAY:-}"
YOURPLACE_MINTER_REGISTRATION="${YOURPLACE_MINTER_REGISTRATION:-}"
if [ -n "$YOURPLACE_MINTER_REGISTRATION" ]; then
  printf '%s' "$YOURPLACE_MINTER_REGISTRATION" | jq -e 'type == "object" and (.chains | type == "array")' >/dev/null
fi
MINTER_REGISTRATION_BASE64=$(printf '%s' "$YOURPLACE_MINTER_REGISTRATION" | base64 | tr -d '\n')

if [ -n "$YOURPLACE_IPFS_GATEWAY" ]; then
  printf '%s' "$YOURPLACE_IPFS_GATEWAY" | jq -Rse 'length <= 253 and test("^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)+$") and (split(".") | all(length <= 63))' >/dev/null || {
    echo 'ERROR: YOURPLACE_IPFS_GATEWAY must be a hostname without scheme, port, or path.' >&2
    exit 1
  }
fi
...
export YOURPLACE_MINTER_REGISTRATION=$(printf '%s' 'MINTER_REGISTRATION_BASE64_PLACEHOLDER' | base64 -d)
if [ -n "$YOURPLACE_MINTER_REGISTRATION" ] && [ -n "$(swapon --show --noheadings)" ]; then
  echo 'ERROR: Disable host swap before provisioning in-memory minter credentials.'
  exit 1
fi
...
docker run -d --name yourplace-gateway --restart unless-stopped --ulimit core=0 -p 127.0.0.1:42430:42430 -e YOURPLACE_MINTER_BOOTSTRAP_ADDR=0.0.0.0:42430 -e YOURPLACE_MINTER_REGISTRATION --log-driver awslogs --log-opt awslogs-region=AWS_REGION_PLACEHOLDER --log-opt awslogs-group=/ec2/yourplace-gateway -p 443:443 -v /opt/YourPlace:/opt/YourPlace -e BASE_RPC_THROTTLE='BASE_RPC_THROTTLE_PLACEHOLDER' -e BASE_RPC_URL='BASE_RPC_URL_PLACEHOLDER' -e CDP_ONRAMP_KEY_NAME='CDP_ONRAMP_KEY_NAME_PLACEHOLDER' -e CDP_ONRAMP_PRIVATE_KEY='CDP_ONRAMP_PRIVATE_KEY_PLACEHOLDER' -e ETHEREUM_RPC_THROTTLE='ETHEREUM_RPC_THROTTLE_PLACEHOLDER' -e ETHEREUM_RPC_URL='ETHEREUM_RPC_URL_PLACEHOLDER' -e YOURPLACE_GATEWAY_CATCHUP='GATEWAY_CATCHUP_PLACEHOLDER' -e YOURPLACE_IPFS_GATEWAY='YOURPLACE_IPFS_GATEWAY_PLACEHOLDER' -e YOURPLACE_IPFS_PINNING_KEY='PINNING_KEY_PLACEHOLDER' -e YOURPLACE_IPFS_PINNING_TYPE='PINNING_TYPE_PLACEHOLDER' -e YOURPLACE_IPFS_PINNING_URL='PINNING_URL_PLACEHOLDER' -e YOURPLACE_ORIGIN='YOURPLACE_ORIGIN_PLACEHOLDER' -e YOURPLACE_SPOTIFY_CLIENT_ID='YOURPLACE_SPOTIFY_CLIENT_ID_PLACEHOLDER' $MYSQL_DSN_ENV ECR_REGISTRY_PLACEHOLDER/yourplace-gateway:latest
...
SCRIPT="${SCRIPT//MINTER_REGISTRATION_BASE64_PLACEHOLDER/$MINTER_REGISTRATION_BASE64}"
SCRIPT="${SCRIPT//YOURPLACE_ORIGIN_PLACEHOLDER/$YOURPLACE_ORIGIN}"
SCRIPT="${SCRIPT//YOURPLACE_IPFS_GATEWAY_PLACEHOLDER/$YOURPLACE_IPFS_GATEWAY}"
