...
# JSON API
### Enrollment
- `yp/1/e:{"e":"url"}` - Enroll/register server gateway URL (to gateway address)

### Posting
- `yp/1/p:{"p":"payload"}` - Post a message (to burn address) 🚢
- `yp/1/pa:{"p":"payload","a":[["ipfs://CID/path.exe","application/vnd.microsoft.portable-executable",4096,"filename.exe"],["ipfs://CID2/path2.jpg","image/jpeg",2048,"filename.jpg"]]}` - Post a message with file attachment(s) [{path, mimetype, size, filename}] (to burn address) 🚢
- `yp/1/pr:{"txh":"txnHash"}` - Repost a post (to burn address)
- `yp/1/pry:{"txh":"txnHash","p":"payload"}` - Reply to a post (to original poster)
- `yp/1/prp:{"txh":"txnHash","p":"payload"}` - Repost a post with a message (to burn address)
...
- `yp/1/fu:{"a":"address", "b":"blockchain"}` - Unfollow an address at a blockchain (to followee address) 🚢
...
- `yp/1/mn:{"n":"name"}` - Update name (to burn address) 🚢
- `yp/1/ma:{"a":"ipfs://CID"}` - Update avatar Img (IPFS) (to burn address) 🚢
- `yp/1/mb:{"b":"ipfs://CID"}` - Update banner Img (IPFS) (to burn address) 🚢
...
- `yp/1/mbot:{"bot":"true"}` = Set profile to bot (to burn address)
### Block
- `yp/1/bl:{"l":"https://block.list.com/list.json"}` - Subscribe to block list (HTTPS/IPFS) (to burn address)
- `yp/1/ba:{"a":"address"}` - Block an address (to blockee address)
- `yp/1/bh:{"h#":"#hashtag"}` - Block a hashtag (to burn address)
- `yp/1/bw:{"w":"word"}` - Block a word (to burn address)
- `yp/1/br:{"r":"^regex$"}` - Block a regex (to burn address)
- `yp/1/bul:{"l":"https://block.list.com/list.json"}` - Unsubscribe to block list (HTTPS/IPFS) (to burn address)
- `yp/1/bua:{"a":"address"}` - Unblock an address (to blockee address)
- `yp/1/buh:{"h#":"#hashtag"}` - Unblock a hashtag (to burn address)
- `yp/1/buw:{"w":"word"}` - Unblock a word (to burn address)
- `yp/1/bur:{"r":"^regex$"}` - Unblock a regex (to burn address)
### Settings
- `yp/1/sa:{"a":"yourplace"}` - Set avatar preference to yourplace (to burn address)
- `yp/1/sa:{"a":"nfdomains"}` - Set avatar preference to nfdomains (to burn address)
- `yp/1/sd:{"d":"yourplace"}` - Set description preference to yourplace (to burn address)
- `yp/1/sd:{"d":"nfdomains"}` - Set description preference to nfdomains (to burn address)
- `yp/1/ss:{"s":"false"}` - Set saved post preference to false (to burn address)
- `yp/1/sap:{"ap":"false"}` - Set autoplay preference to false (to burn address)