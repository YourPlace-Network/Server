...
func (db *SQLite) createTables(ctx context.Context) error {
	...
	*/
	...
	tables := map[string]string{
		...
		"user_notification_seen": "CREATE TABLE IF NOT EXISTS user_notification_seen (userAddress TEXT, userBlockchain TEXT, lastSeenAt INTEGER DEFAULT 0, PRIMARY KEY (userAddress, userBlockchain))",
		"user_notifications":     "CREATE TABLE IF NOT EXISTS user_notifications (id TEXT PRIMARY KEY, userAddress TEXT, userBlockchain TEXT, fromAddress TEXT, fromBlockchain TEXT, type TEXT, targetTxHash TEXT DEFAULT '', reactionType TEXT DEFAULT '', timestamp INTEGER DEFAULT 0, dismissed INTEGER DEFAULT 0)",
...
// --- Search --- //
func (db *SQLite) SearchGetPosts(query string, limit int, offset int) []map[string]interface{} {
	var posts []map[string]interface{}
	...
	sqlQuery := fmt.Sprintf("SELECT txHash, parentHash, timestamp, data, fromAddress, blockchain FROM (%s) ORDER BY timestamp DESC LIMIT ? OFFSET ?", strings.Join(unionParts, " UNION ALL "))
...
func (db *SQLite) SearchGetProfiles(query string, limit int, offset int) []map[string]interface{} {
	...
	params = append(params, limit, offset)
	sqlQuery := fmt.Sprintf("SELECT address, blockchain FROM (%s) GROUP BY address, blockchain LIMIT ? OFFSET ?", strings.Join(unionParts, " UNION ALL "))
	rows, err := db.runParamSQLSelect(sqlQuery, params...)
	if err != nil {
...
func (db *SQLite) DiscoverGetTopByFollowers(limit int) []map[string]interface{} {
	...
	sqlQuery := fmt.Sprintf("SELECT followeeAddress, followeeBlockchain, COUNT(*) as follower_count FROM (%s) GROUP BY followeeAddress, followeeBlockchain ORDER BY follower_count DESC LIMIT ?", strings.Join(unionParts, " UNION ALL "))
	rows, err := db.runParamSQLSelect(sqlQuery, limit)
	if err != nil {
...
func (db *SQLite) OnchainMBot(blockchain string, address string, bot bool, timestamp uint64) {
	botInt := 0
	if bot {
		botInt = 1
	}
...
func (db *SQLite) OnchainMNsfw(blockchain string, address string, nsfw bool, timestamp uint64) {
	nsfwInt := 0
	if nsfw {
		nsfwInt = 1
	}
...
func (db *SQLite) HasUserCommented(parentTxHash string, blockchain string, address string) bool {
	var unionParts []string
	var params []interface{}
	for _, chain := range core.ValidNetworks {
	...
	query := fmt.Sprintf("SELECT COUNT(*) FROM (%s) LIMIT 1", strings.Join(unionParts, " UNION ALL "))
...
func (db *SQLite) GetReactionCounts(targetTxHash string, blockchain string) map[string]interface{} {
	...
	query := fmt.Sprintf("SELECT reactionType, COUNT(DISTINCT fromAddress) as count FROM (%s) GROUP BY reactionType", strings.Join(unionParts, " UNION ALL "))
	rows, err := db.runParamSQLSelect(query, params...)
	if err != nil {
...
func (db *SQLite) GetUserReactions(targetTxHash string, blockchain string, fromAddress string) map[string]string {
	...
	query := fmt.Sprintf(`SELECT reactionType FROM (
		...
		FROM (%s)
	) WHERE rn = 1`, strings.Join(unionParts, " UNION ALL "))
	rows, err := db.runParamSQLSelect(query, params...)
	if err != nil {
		return result
	}
...
// --- User Notifications --- //
func (db *SQLite) UserNotificationInsert(id string, userAddress string, userBlockchain string, fromAddress string, fromBlockchain string, notifType string, targetTxHash string, reactionType string, timestamp uint64) {
	_, err := db.runParamSQLUpdate("INSERT INTO user_notifications (id, userAddress, userBlockchain, fromAddress, fromBlockchain, type, targetTxHash, reactionType, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO NOTHING", id, userAddress, userBlockchain, fromAddress, fromBlockchain, notifType, targetTxHash, reactionType, timestamp)
	if err != nil {
		core.LogDebug("Could not insert user notification: " + err.Error())
	}
}
func (db *SQLite) UserNotificationClearAll(userAddress string, userBlockchain string) {
...
func (db *SQLite) UserNotificationCleanup() {
...
func (db *SQLite) UserNotificationDismiss(id string) {
...
func (db *SQLite) UserNotificationGet(userAddress string, userBlockchain string, limit int, offset int) []map[string]string {
	cutoff := core.TimestampMinusDays(core.GetTimestamp(), 30)
...
func (db *SQLite) UserNotificationGetCount(userAddress string, userBlockchain string, since uint64) int64 {
	cutoff := core.TimestampMinusDays(core.GetTimestamp(), 30)
...
func (db *SQLite) UserNotificationGetSeen(userAddress string, userBlockchain string) uint64 {
...
func (db *SQLite) UserNotificationUpdateSeen(userAddress string, userBlockchain string, timestamp uint64) {
	_, err := db.runParamSQLUpdate("INSERT INTO user_notification_seen (userAddress, userBlockchain, lastSeenAt) VALUES (?, ?, ?) ON CONFLICT (userAddress, userBlockchain) DO UPDATE SET lastSeenAt = excluded.lastSeenAt", userAddress, userBlockchain, timestamp)
	if err != nil {
