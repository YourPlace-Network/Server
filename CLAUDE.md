...
- For modal dialog content, the default styling in `src/scss/components/modalDialog.scss` is intentionally centered so that short error/status messages and loading/progress modals read well. When a modal needs multi-paragraph or help-style long-form content, wrap the HTML in a `<div class='modalBodyLeft'>...</div>` to opt into left-aligned body text. Do not flip the global default alignment — it would visually regress the short-message majority of modal call sites.
